...
LetterSpace supports various environment variables for configuration:
...
- **NODE_ENV**: Environment mode (default: development, use "production" for production deployments)
...
The webhook transformer variables control resource limits for custom webhook transformation code execution. Lower values provide better security and resource protection, while higher values allow for more complex transformations.
