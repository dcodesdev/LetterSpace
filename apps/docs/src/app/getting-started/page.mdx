...
1.  **Create a `docker-compose.yaml` file:**
    ...
    services:
      ...
      backend:
        image: ghcr.io/dcodesdev/letterspace # Use the pre-built image
        restart: always
        depends_on:
          - db
        ports:
...
To access the LetterSpace dashboard securely via a domain name (e.g., `https://newsletter.yourdomain.com`), you can use a reverse proxy like Nginx or Caddy.
...
```bash
newsletter.yourdomain.com { # Replace with your actual domain
    reverse_proxy localhost:5000
}
```

> Replace `newsletter.yourdomain.com` with your actual domain. Caddy will automatically provision an SSL certificate for it.
...
Wait a minute or two for Caddy to provision the SSL certificate, then access the LetterSpace dashboard securely at `https://newsletter.yourdomain.com`.
